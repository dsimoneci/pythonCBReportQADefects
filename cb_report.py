...
try:
    ...
    idx = 0
    now = datetime.now()
    dt1_string = now.strftime("%m%d%Y%H%M%S")
    dt2_string = now.strftime("%m/%d/%Y %H:%M")
    idx = 1
    ...
    try:
        ...
        cur.execute("select distinct u.id, u.name, u.email, u.phone, u.created_at, count(cu.client_id) as \"client count\", julianday('now') - julianday(u.created_at) as \"account age\" from \"user\" as u, client_user as cu where u.id = cu.user_id and u.is_admin=1 and julianday('now') - julianday(u.created_at) < 31 group by u.id order by u.name")
        # get the results
        ...
        if rowStuff is not None:
            f.write(tabulate(rowStuff, headers=["ID","Name","Email","Created On","Phone","Client Count","Account Age"],tablefmt='fancy_grid'))
            f.write("\n\n")
        else:
            f.write("** NO DATA **")
        ...
        ## USER LISTING
        # execute a query
        cur.execute("select distinct u.id, u.name, u.email, u.phone, u.created_at, count(cu.client_id) as \"client count\" from \"user\" as u, client_user as cu where u.id = cu.user_id and u.is_admin=1 group by u.id order by u.name")
        # get the results
        rowStuff = cur.fetchall()

        # how many users
        for rows in rowStuff:
            idx = idx + 1

        # tabulate the results into a pretty table     
        f.write("ClientBoxx Users: "+str(idx)+"\n")
        # write 'em out
        if rowStuff is not None:
        ...
        ## STORAGE DETAILS
        # execute a query
        cur.execute("select distinct u.id, u.name, u.email, u.phone, u.created_at, count(at.id)-1, round(sum(at.file_size)/1000000000,2) from \"user\" as u, attachment as at where u.id = at.user_id group by u.id order by u.name")
        # get the results
        rowStuff = cur.fetchall()
        # tabulate the results into a pretty table     
